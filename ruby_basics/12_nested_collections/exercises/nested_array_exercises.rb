...
def delete_row_from_chart(chart, row_index)
  ...
  # then return the chart
  chart[row_index].pop(chart[row_index].length)
  chart.select { |array| array != [] }
end
...
def count_empty_seats(chart)
  ...
  nil_count = 0
  chart.each do |nested_array|
    nil_count += nested_array.count nil
  end
  nil_count
end
...
def find_favorite(array_of_hash_objects)
  ...
  favorite_hash_object = array_of_hash_objects.select do |hash_object|
    hash_object[:is_my_favorite?] == true
  end
  favorite_hash_object[0]
end
